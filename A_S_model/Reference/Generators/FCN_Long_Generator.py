...
start_time = time.time() #timers are fun!

'inputs'
x_dim=300 #number of entries per row
y_dim=300 #number of rows
Func=[5*sin(z)+40,5*cos(z)+z+3] # the function(s) you are mapping. Note: if you are using a non cont. and/or non diff.able function this code can produce errors (also what is wrong with you?! )
Map_name='Fn_test_4' #name of map file you want
...
for Fcn in Func: #does the mapping for each function
    ...
    for row in Matrix: # seperates out each row
        ...
        print(R+1,len(Func),y) #lets you see progress while it runs
...
print('Done')
print("--- %s seconds ---" % (time.time() - start_time)) #what time is it? 
#497.62652492523193 s
#512.3737421035767
'''
